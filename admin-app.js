...
async function loadData() {
    if (isConnected) {
        try {
            ...
            const {data:s} = await supabase.from('parking_slots').select('*');
            adminState.slots = s&&s.length?s:generateSlots();
            const {data:l} = await supabase.from('parking_logs').select('*, users(full_name)').order('timestamp',{ascending:false}).limit(30);
            if(l) adminState.logs = l;
        } catch(e) { console.error(e); adminState.users = demoUsers; adminState.slots = generateSlots(); }
    } else {
...
function renderAdmin() {
    ...
    const authorized = adminState.users.filter(u=>u.authorization_status==='AUTHORIZED');
    ...
    if(el('adminStatEntries')) el('adminStatEntries').textContent = adminState.logs.length || 42;
    ...
    if(el('usersTable')) {
        ...
        el('usersTable').innerHTML = filtered.map(u=>`<tr class="hover:bg-white/60 border-b border-slate-100/50"><td class="p-4 font-bold text-slate-800">${u.full_name}</td><td class="p-4"><span class="px-2 py-0.5 rounded text-xs font-bold bg-slate-200 text-slate-700 uppercase">${u.role}</span><div class="text-xs text-slate-500 mt-1">${u.program||'--'} • ${u.section||'--'}</div></td><td class="p-4 font-mono text-xs text-slate-500">${u.rfid_uid||'<span class=text-yellow-600>Not Assigned</span>'}</td><td class="p-4"><div class="font-semibold text-slate-700">${u.vehicle_type||'--'} - ${u.vehicle_model||'--'}</div></td><td class="p-4 font-mono font-bold text-slate-700">${u.plate_number||'--'}</td><td class="p-4 text-center"><span class="px-2 py-1 rounded text-[10px] font-bold bg-green-100 text-green-700">ACTIVE</span></td><td class="p-4 text-right whitespace-nowrap"><button onclick="openUserModal('${u.id}')" class="p-1 text-slate-400 hover:text-charm-dark"><i data-lucide="edit" class="w-4 h-4"></i></button><button onclick="deleteUser('${u.id}')" class="p-1 text-slate-400 hover:text-red-500 ml-2"><i data-lucide="trash" class="w-4 h-4"></i></button></td></tr>`).join('');
    }
...
window.approveUser = async function(id) {
    if(isConnected) { try { await supabase.from('users').update({authorization_status:'AUTHORIZED'}).eq('id',id); await loadData(); } catch(e){ showToast('Error: '+e.message,'error'); } }
    else { const u = adminState.users.find(x=>x.id===id); if(u){u.authorization_status='AUTHORIZED';} adminState.pendingUsers = adminState.users.filter(u=>u.authorization_status==='PENDING'); renderAdmin(); }
    showToast('User approved successfully!','success');
};

window.denyRegistration = async function(id) {
    if(!confirm('Deny this registration?')) return;
    if(isConnected) { try { await supabase.from('users').update({authorization_status:'DENIED'}).eq('id',id); await loadData(); } catch(e){ showToast('Error: '+e.message,'error'); } }
    else { adminState.users = adminState.users.filter(u=>u.id!==id); adminState.pendingUsers = adminState.users.filter(u=>u.authorization_status==='PENDING'); renderAdmin(); }
    showToast('Registration denied.','warning');
};

window.deleteUser = async function(id) {
    if(!confirm('Delete this user?')) return;
    if(isConnected) { try { await supabase.from('users').delete().eq('id',id); await loadData(); } catch(e){ showToast('Error: '+e.message,'error'); } }
    else { adminState.users = adminState.users.filter(u=>u.id!==id); renderAdmin(); }
...
window.openUserModal = function(id) {
    ...
    if(id) {
        ...
        if(u) { document.getElementById('formName').value=u.full_name; document.getElementById('formUid').value=u.rfid_uid||''; document.getElementById('formRole').value=u.role; document.getElementById('formAge').value=u.age||''; document.getElementById('formSex').value=u.sex||'Male'; document.getElementById('formAddress').value=u.address||''; document.getElementById('formProgram').value=u.program||''; document.getElementById('formSection').value=u.section||''; document.getElementById('formVehType').value=u.vehicle_type||'Car'; document.getElementById('formPlate').value=u.plate_number||''; document.getElementById('formVehModel').value=u.vehicle_model||''; document.getElementById('formVehColor').value=u.vehicle_color||''; }
    } else { document.getElementById('modalTitle').textContent='Register New User'; document.getElementById('userForm').reset(); }
    m.classList.remove('hidden');
    setTimeout(()=>{m.classList.remove('opacity-0');document.getElementById('userModalContent').classList.remove('scale-95');},10);
    lucide.createIcons();
};

window.closeUserModal = function() {
    const m = document.getElementById('userModal');
    m.classList.add('opacity-0'); document.getElementById('userModalContent').classList.add('scale-95');
    setTimeout(()=>m.classList.add('hidden'),300);
};

window.saveUser = async function() {
    const name = document.getElementById('formName').value;
    const uid = document.getElementById('formUid').value;
    const role = document.getElementById('formRole').value;
    if(!name||!role) { showToast('Name and Role are required.','warning'); return; }
    const userData = { full_name:name, rfid_uid:uid.toUpperCase().trim()||null, role, age:document.getElementById('formAge').value?parseInt(document.getElementById('formAge').value):null, sex:document.getElementById('formSex').value, address:document.getElementById('formAddress').value, program:document.getElementById('formProgram').value, section:document.getElementById('formSection').value, vehicle_type:document.getElementById('formVehType').value, plate_number:document.getElementById('formPlate').value.toUpperCase().trim()||null, vehicle_model:document.getElementById('formVehModel').value, vehicle_color:document.getElementById('formVehColor').value, authorization_status:'AUTHORIZED' };
    const userId = document.getElementById('formUserId').value;
    try {
        if(isConnected) { if(userId){await supabase.from('users').update(userData).eq('id',userId);}else{await supabase.from('users').insert([userData]);} await loadData(); }
        else { if(userId){const i=adminState.users.findIndex(u=>u.id===userId);if(i>-1)Object.assign(adminState.users[i],userData);}else{adminState.users.push({...userData,id:'local_'+Date.now(),created_at:new Date().toISOString()});} renderAdmin(); }
        closeUserModal(); showToast('User saved successfully!','success');
...
function initCharts() {
    ...
    if(ctx1) new Chart(ctx1,{type:'doughnut',data:{labels:['Students','Faculty','Staff'],datasets:[{data:[65,20,15],backgroundColor:['#1F6B4F','#B7D8B0','#F4C542'],borderWidth:0,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{boxWidth:12,usePointStyle:true}}},cutout:'70%'}});
